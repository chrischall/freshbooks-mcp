...
export function registerEstimateTools(
  ...
  server.registerTool(
    ...
    async ({ id, confirmToken }, ctx) => {
      ...
      return minifiedResult({
        ...result,
        accepted: isAccepted(result.estimate),
      });
    },
  );
  ...
  server.registerTool(
    "freshbooks_update_estimate",
    {
      ...
      inputSchema: z.object({
        ...
        description: z
          .string()
          .optional()
          .describe("Estimate description / summary line"),
        presentation: z
        ...
        lines: z
          ...
          .optional()
          .describe(
            "Full replacement line set. Include lineid on a line to update it in place.",
          ),
        estimate_number: z
          .string()
          .optional()
          .describe('User-visible estimate number, e.g. "0000654"'),
        ...
        discount_value: z
          .string()
          .optional()
          .describe('Discount percentage as a decimal string, e.g. "30"'),
        customerid: z
          .number()
          .int()
          .positive()
          .optional()
          .describe("Client the estimate is for"),
        fields: z
          .record(z.string(), z.unknown())
          .optional()
          .describe(
    ...
    async ({ id, confirmToken, fields, ...rest }, ctx) => {
      ...
      return minifiedResult({
        ...(await verify(client, id, before, written, sentFields)),
        sentFields,
      });
    },
  );
    ...
    "freshbooks_send_estimate",
    {
      ...
      inputSchema: z.object({
        ...
        email_recipients: z
          ...
          .optional()
          .describe(
            "Recipient addresses. Omit to let FreshBooks send to the client's address on file.",
          ),
...
async function assertAccountIdShape(client: FreshbooksClient): Promise<void> {
  ...
  if (
    identity.accountId === String(identity.businessId) ||
    identity.accountId === identity.businessUuid
  ) {
 ...
 */
async function readEstimateForWrite(
  client: FreshbooksClient,
  id: number,
): Promise<unknown> {
  try {
    ...
    const isMiss =
      ...
      (err instanceof McpToolError && /returned 404/.test(err.message));
    if (!isMiss) throw err;
...
async function verify(
  ...
  const watched = [...new Set([...STATE_KEYS, ...watch])].filter(
    (k) => !k.startsWith("action_"),
  );
  const changedFields = watched.filter(
...
function stateOf(estimate: unknown): Record<string, unknown> {
  ...
  return Object.fromEntries(
    STATE_KEYS.filter((k) => k in e).map((k) => [k, e[k]]),
  );
}
...
function isAccepted(estimate: unknown): boolean {
  ...
  return [e.display_status, e.ui_status].some(
    (v) => v === "accepted" || v === "invoiced",
  );
}
